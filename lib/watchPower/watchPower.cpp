...
WatchPower::WatchPower(SoftwareSerial &_refSer){
    refSer = &_refSer;
...
WatchPower::WatchPower(SoftwareSerial &_refSer, bool _conditioningEnabled) : WatchPower(_refSer){
...
bool WatchPower::setOutputSourcePriority(OutputSourcePriorities prio){
    ...
    /* Make Command */
    sprintf(command, "POP%02u", static_cast<unsigned int>(prio));
    appendCRC(command);
